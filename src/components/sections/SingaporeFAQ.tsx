...
interface SingaporeFAQProps {
  content?: FAQContent;
}

const defaultSingaporeFaqData: FAQType[] = [
  {
    id: '1',
    question: "What's included in the Travlogers Singapore package?",
    answer: "Our Singapore package includes hotel stays, daily breakfast & dinner, private cab for sightseeing, and photography. Universal Studios tickets, cruise bookings, and adventure activities can be added as extras. You'll also get 24/7 trip support from our team."
  },
  {
    id: '2',
    question: "Is photography included in the group trip?",
    answer: "Yes, professional photography is included in all our Singapore group trips. Our experienced photographers will capture your best moments throughout the journey, including stunning cityscapes, Universal Studios, and Marina Bay Sands, and you'll receive a curated collection of high-quality photos after the trip."
  },
  {
    id: '3',
    question: "How does the booking process work?",
    answer: "The booking process is simple: 1) Choose your preferred Singapore package and dates, 2) Pay a small booking amount to secure your spot, 3) Complete the remaining payment before the trip, 4) Receive your detailed itinerary and travel documents. Our team will guide you through each step."
  },
  {
    id: '4',
    question: "Will someone assist us during the trip?",
    answer: "Absolutely! You'll have a dedicated trip coordinator who will be available 24/7 throughout your Singapore journey. Additionally, our local guides will accompany you to all major attractions and provide insights about Singapore's culture, history, and hidden gems."
  },
  {
    id: '5',
    question: "Do you arrange adventure activities or surprises for couples?",
    answer: "Yes, we specialize in creating magical experiences in Singapore! We can arrange adventure activities like cable car rides, SkyHelix, and Sentosa attractions. For couples, we offer romantic surprises like candlelight dinners at Marina Bay Sands, private cruise experiences, and special photography sessions at scenic locations like Gardens by the Bay."
  }
];

const SingaporeFAQ = ({ content }: SingaporeFAQProps) => {
  ...
  const faqData = React.useMemo(() => {
    if (!content?.items || content.items.length === 0) {
      return defaultSingaporeFaqData;
    }
    
    // Smart merging: combine default items with CMS items
    return defaultSingaporeFaqData.map(defaultItem => {
