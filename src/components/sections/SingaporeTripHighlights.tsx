...
interface SingaporeTripHighlightsProps {
  content?: TripHighlightsContent;
}

const singaporeHighlights: HighlightImage[] = [
  { 
    id: '1', 
    src: highlight1, 
    alt: 'Marina Bay Sands - Iconic Singapore Skyline'
  },
  { 
    id: '2', 
    src: highlight2, 
    alt: 'Universal Studios Singapore - Thrilling Theme Park'
  },
  { 
    id: '3', 
    src: highlight3, 
    alt: 'Gardens by the Bay - Supertree Grove & Flower Dome'
  },
  { 
    id: '4', 
    src: highlight1, 
    alt: 'Sentosa Island - Cable Car & Adventure Cove'
  },
  { 
    id: '5', 
    src: highlight2, 
    alt: 'Singapore Flyer - World\'s Largest Observation Wheel'
  },
  { 
    id: '6', 
    src: highlight3, 
    alt: 'Night Safari - World\'s First Nocturnal Wildlife Park'
  },
];

const SingaporeTripHighlights = ({ content }: SingaporeTripHighlightsProps) => {
  ...
  const highlightsData = React.useMemo(() => {
    if (!content?.highlights || content.highlights.length === 0) {
      return singaporeHighlights;
    }
    
    // Smart merging: combine default items with CMS items
    return singaporeHighlights.map(defaultItem => {
        ...
        <LazyLoad animationType="fade" delay={0.2}>
          <div className="text-center mb-8 md:mb-12">
            ...
            <p className="text-gray-600 text-base md:text-lg max-w-2xl mx-auto">
              {content?.subheading || "Experience the most breathtaking destinations that make Singapore truly magical"}
            </p>
          </div>
        </LazyLoad>
...
export default SingaporeTripHighlights; 