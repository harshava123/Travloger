...
interface MeghalayaTripHighlightsProps {
  content?: TripHighlightsContent;
}

const meghalayaHighlights: HighlightImage[] = [
  { 
    id: '1', 
    src: highlight1, 
    alt: 'Double-Decker Living Root Bridge - Nongriat Village'
  },
  { 
    id: '2', 
    src: highlight2, 
    alt: 'Nohkalikai Falls - India\'s Tallest Waterfall'
  },
  { 
    id: '3', 
    src: highlight3, 
    alt: 'Dawki River - Crystal Clear Waters of Umngot'
  },
  { 
    id: '4', 
    src: highlight1, 
    alt: 'Mawlynnong Village - Asia\'s Cleanest Village'
  },
  { 
    id: '5', 
    src: highlight2, 
    alt: 'Laitlum Grand Canyon - Panoramic Cliff Views'
  },
  { 
    id: '6', 
    src: highlight3, 
    alt: 'Mawsmai Caves - Limestone Cave Exploration'
  },
];

const MeghalayaTripHighlights = ({ content }: MeghalayaTripHighlightsProps) => {
  ...
  const highlightsData = React.useMemo(() => {
    if (!content?.highlights || content.highlights.length === 0) {
      return meghalayaHighlights;
    }
    
    // Smart merging: combine default items with CMS items
    return meghalayaHighlights.map(defaultItem => {
        ...
        <LazyLoad animationType="fade" delay={0.2}>
          <div className="text-center mb-8 md:mb-12">
            <h2 className={cn(
              ...
              {content?.heading || (
                <>
                  Discover Meghalaya&apos;s{' '}
                  <span className="text-[#134956]">Hidden Gems</span>
                </>
              )}
            </h2>
            <p className="text-gray-600 text-base md:text-lg max-w-2xl mx-auto">
              {content?.subheading || "Experience the most breathtaking destinations that make Meghalaya truly magical"}
            </p>
          </div>
        </LazyLoad>
...
export default MeghalayaTripHighlights; 