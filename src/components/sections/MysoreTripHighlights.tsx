...
interface MysoreTripHighlightsProps {
  content?: TripHighlightsContent;
}

const mysoreHighlights: HighlightImage[] = [
  { 
    id: '1', 
    src: highlight1, 
    alt: 'Mysore Palace - Royal Heritage and Grand Architecture'
  },
  { 
    id: '2', 
    src: highlight2, 
    alt: 'Chamundi Hills - Sacred Temple and Panoramic Views'
  },
  { 
    id: '3', 
    src: highlight3, 
    alt: 'Coorg Coffee Estates - Misty Mountains and Aromatic Coffee'
  },
  { 
    id: '4', 
    src: highlight1, 
    alt: 'Brindavan Gardens - Musical Fountain and Evening Magic'
  },
  { 
    id: '5', 
    src: highlight2, 
    alt: 'Wayanad Forests - Tribal Culture and Wilderness'
  },
  { 
    id: '6', 
    src: highlight3, 
    alt: 'Ooty Hill Station - Tea Gardens and Toy Train'
  },
];

const MysoreTripHighlights = ({ content }: MysoreTripHighlightsProps) => {
  ...
  const highlightsData = React.useMemo(() => {
    if (!content?.highlights || content.highlights.length === 0) {
      return mysoreHighlights;
    }
    
    // Smart merging: combine default items with CMS items
    return mysoreHighlights.map(defaultItem => {
        ...
        <LazyLoad animationType="fade" delay={0.2}>
          <div className="text-center mb-8 md:mb-12">
            ...
            <p className="text-gray-600 text-base md:text-lg max-w-2xl mx-auto">
              {content?.subheading || "Experience the most breathtaking destinations that make Mysore truly magical"}
            </p>
          </div>
        </LazyLoad>
...
export default MysoreTripHighlights; 